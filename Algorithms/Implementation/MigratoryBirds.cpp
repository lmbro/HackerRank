...
int main(){
    ...
    short id;
    short max_id_index=0;
    int n;
    int id_count[5] = { 0, 0, 0, 0, 0 };
    ...
    for(int i=1; i<5; i++){ // max_id_index initialized as 0
        if( id_count[i] > id_count[max_id_index] ){
            max_id_index = i;
        }
    }
    std::cout << max_id_index+1 << std::endl;
    return 0;
}
